...
class AuthProvider extends ChangeNotifier {
  ...
  bool get isLoading =>
      _status == AuthStatus.loading || _status == AuthStatus.authenticating;
    ...
    try {
      ...
      if (firebaseUser != null) {
        ...
        if (user != null) {
          ...
          log('[AuthProvider] Could not resolve user — signing out.');
          await _authService.signOut();
          _status = AuthStatus.unauthenticated;
        }
      } else {
        log('[AuthProvider] No active session.');
        _status = AuthStatus.unauthenticated;
      }
    } catch (e) {
  ...
  String _parseError(String code) {
    ...
    if (c.contains('no-firestore-record') ||
        c.contains('user-not-found') ||
        c.contains('error_user_not_found')) {
      return 'No account found for this email.';
    }
    if (c.contains('wrong-password') || c.contains('invalid_password')) {
      return 'Incorrect password.';
    }
    if (c.contains('invalid-credential') ||
        c.contains('invalid_login_credentials')) {
      return 'Incorrect email or password.';
    }
    if (c.contains('invalid-email')) {
      return 'Invalid email address.';
    }
    if (c.contains('user-disabled')) {
      return 'This account has been disabled.';
    }
    if (c.contains('too-many-requests')) {
      return 'Too many attempts. Please try again later.';
    }
    if (c.contains('network-request-failed')) {
      return 'No internet connection.';
    }
    // Firestore transient errors — these are handled internally with retry
    // but surface here only if all retries failed AND cache was empty.
    if (c.contains('unavailable') ||
        c.contains('firestore-unavailable') ||
        c.contains('cloud_firestore')) {
