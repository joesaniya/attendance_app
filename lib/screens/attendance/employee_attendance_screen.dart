...
// ── Scan states ────────────────────────────────────────────────────────────────
enum ScanState { scanning, faceDetected, matching, matched, noMatch, result }
...
class _EmployeeAttendanceScreenState extends State<EmployeeAttendanceScreen>
  ...
  // ── Stream guard: only one frame processed at a time ──────────────────────────
  ...
  Future<void> _initCamera() async {
    try {
      ...
      final controller = CameraController(
        front,
        ResolutionPreset.medium, // medium = less memory pressure than high
        enableAudio: false,
        imageFormatGroup: ImageFormatGroup
            .nv21, // explicit format avoids format-detection GC stalls
      );
  ...
  void _startDetection() {
    ...
    controller.startImageStream((CameraImage image) async {
      // Hard guards — drop frame immediately if any condition fails
      ...
      try {
        ...
        // detectFacesFromCameraImage now owns the byte buffer before returning
        ...
        if (faces.isNotEmpty) {
          if (mounted) setState(() => _scanState = ScanState.faceDetected);
          ...
          if (_scanState == ScanState.faceDetected && mounted) {
            setState(() => _scanState = ScanState.scanning);
          }
        }
      } catch (e) {
        debugPrint('[Detection] frame error: $e');
      } finally {
        _isDetecting = false;
      }
    });
  ...
  Future<void> _autoMatchFace(Face face) async {
    ...
    if (mounted) setState(() => _scanState = ScanState.matching);
    ...
    if (matched != null) {
      // Stop stream BEFORE setState to avoid texture-invalid frames
      ...
      if (mounted) setState(() => _scanState = ScanState.noMatch);
  ...
  // ── Load attendance and show result ──────────────────────────────────────────
  Future<void> _loadAndShowResult(
    ...
    if (isAuto && !isLoggedIn && !isCompleted) {
      // Capture photo while camera is still alive, before we switch to result UI
      XFile? autoPhoto = await _capturePhoto();
      ...
      await _markAttendance(true, preCapturedPhoto: autoPhoto);
    } else {
      if (_disposed || !mounted) return;
      setState(() {
        _existingAttendance = attendance;
  ...
  // ── Safe photo capture helper ─────────────────────────────────────────────────
  Future<XFile?> _capturePhoto() async {
    final controller = _cameraController;
    if (controller == null || !controller.value.isInitialized) return null;

    try {
      // Ensure stream is stopped before taking picture (required by camera plugin)
      if (controller.value.isStreamingImages) {
        await controller.stopImageStream();
        _streamActive = false;
  ...
  // ── Mark attendance ───────────────────────────────────────────────────────────
  Future<void> _markAttendance(bool isLogin, {XFile? preCapturedPhoto}) async {
    if (_matchedEmployee == null || _disposed) return;
    if (mounted) setState(() => _isProcessing = true);

    final photo = preCapturedPhoto ?? await _capturePhoto();
    ...
    if (success) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            isLogin ? 'Check-In successful.' : 'Check-Out successful.',
          ),
      ...
      final msg = (error == 'Already logged in today')
  ...
  void dispose() {
    ...
    if (controller != null) {
      if (controller.value.isStreamingImages) {
        controller.stopImageStream().catchError((_) {}).then((_) {
          controller.dispose();
        });
      } else {
        controller.dispose();
  ...
  Widget _buildScannerScreen() {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Camera preview — only mount when initialized and not in result state
                      ...
                      onTap: _isProcessing
                          ...
                          : () async {
                              final employees = await context
                                  .read<EmployeeProvider>()
                                  .getAllEmployeesForAttendance();
                              if (mounted) _showManualSelectSheet(employees);
                            },
                      child: Container(
                        height: 52,
                        decoration: BoxDecoration(
...
class _EmployeeAttendanceScreenState extends State<EmployeeAttendanceScreen>
  ...
  Future<void> _autoMatchFace(Face face) async {
    ...
    if (matched != null) {
      ...
      // Delay to let UI show matched state before capturing
      await Future.delayed(const Duration(milliseconds: 600));
      await _loadAndShowResult(matched, isAuto: true);
    } else {
  ...
  Future<void> _loadAndShowResult(EmployeeModel employee, {bool isAuto = false}) async {
    ...
    if (isAuto && !isLoggedIn && !isCompleted) {
      // Must take picture BEFORE switching UI to Result (which destroys CameraPreview surface)
      XFile? autoPhoto;
      if (_cameraController != null && _cameraController!.value.isInitialized) {
        try {
          ...
          autoPhoto = await _cameraController!.takePicture();
        } catch (e) {
          debugPrint('Failed to capture auto photo: $e');
        }
      }
  ...
  Future<void> _markAttendance(bool isLogin, {XFile? preCapturedPhoto}) async {
    ...
    XFile? photo = preCapturedPhoto;
    if (photo == null && _cameraController != null && _cameraController!.value.isInitialized) {
      try {
        if (_cameraController!.value.isStreamingImages) {
          await _cameraController!.stopImageStream();
                    ...
                    children: [
                      if (!isLoggedIn)
                        GradientButton(
                          ...
                          isLoading: att.isLoading || _isProcessing,
                          onTap: () async {
                            // Ensure camera stream is stopped before manual capture
                            try {
                              if (_cameraController?.value.isStreamingImages == true) {
                                await _cameraController?.stopImageStream();
                              }
